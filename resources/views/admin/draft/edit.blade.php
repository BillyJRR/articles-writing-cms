...
<div class="mx-4">
    ...
    <form class="row g-3" action="{{ route('drafts.update', ['draft' => $draft])}}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="col-md-6">
            <label class="form-label" for="title">Título:</label>
            <input id="title" type="text" class="form-control" name="title" value="{{ $draft->title }}" oninput="generateSlug()" required>
        </div>
        <div class="col-md-6">
            <label class="form-label" for="slug">Slug:</label>
            <input id="slug" type="text" class="form-control" name="slug" value="{{ $draft->slug }}" readonly>
        </div>
        <div class="col-md-6">
            <label class="form-label" for="body">Descripción:</label>
            <textarea id="body" class="form-control" name="body">{{ $draft->body }}</textarea>
        </div>
        {{-- <div class="col-md-6">
            ...
            <select class="form-select" id="inputGroupSelect01" name="author_id" required>
                ...
                <option option value={{ $author->id }} @if($author->id == $draft->author_id) selected @endif>{{ $author->name }}</option>
                @endforeach
            </select>
        </div> --}}
        <div class="col-md-6">
            ...
            <select class="form-select" id="author" multiple name="authors[]" required>
                @foreach($authors as $author)
                <option value="{{ $author->id }}" @if(in_array($author->id, $draft->authors->pluck('id')->toArray())) selected @endif>{{ $author->name }}</option>
                @endforeach
            </select>
        </div>
        <div class="col-md-6">
            ...
            <select class="form-select" id="category" multiple name="categories[]" onchange="updateCategorySelection(event)" required>
                ...
                <option value="{{ $category->id }}" @if(in_array($category->id, $draft->categories->pluck('id')->toArray())) selected @endif>{{ $category->name }}</option>
                @endforeach
            </select>
        </div>
        <div class="col-md-6">
            ...
            <div class="py-2">
                ...
                <img src="{{ asset('storage')."/".$draft->image }}" alt="Imagen actual" width="150">
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const selectedOptions = Array.from(categorySelect.options);
